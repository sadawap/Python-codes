            ...
            while True:
                ...
                if apps:
                    ...
                    for app in apps:
                        ...
                        app_deleted = 'No' if app_status == 'InService' else 'Yes'
                        csv_data.append([profile_name, app_name, app_status, app_deleted])
                if not app_next_token:
                    break

            if not apps_found:
                csv_data.append([profile_name, "No apps found", "N/A", "No apps running"])

        # Write data to CSV
        with open('sagemaker_profiles_and_apps.csv', mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["User Profile", "App Name", "Status", "App Deleted"])
